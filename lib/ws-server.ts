...
export class WebSocketServerManager extends EventEmitter {
    ...
    api = {
        ...
        subscribe: (topic: string, handler: SubscriptionHandler, middlewares?: Middleware[]) =>
            this.registerSubscription(topic, handler, middlewares),
    }
    constructor() {
        super()
    }
    ...
    registerSubscription(topic: string, handler: SubscriptionHandler, middlewares: Middleware[] = []) {
        const route = `/subscribe/${topic}`
        const matchFn = match(route, {decode: decodeURIComponent})

        // Enhance context with subscription methods
        const subscriptionMiddleware: Middleware = async(ctx, next) => {
            const subscriptionCtx: SubscriptionContext = {
                ...ctx,
                subscribe: (topic: string) => {
                    if (!this.subscriptions[topic]) {
                        this.subscriptions[topic] = new Set()
                    }
                    this.subscriptions[topic].add(ctx.ws)

                    if (!this.clientSubscriptions.has(ctx.ws)) {
                        this.clientSubscriptions.set(ctx.ws, new Set())
                    }
                    this.clientSubscriptions.get(ctx.ws)?.add(topic)
                },
                unsubscribe: (topic: string) => {
                    this.subscriptions[topic]?.delete(ctx.ws)
                    this.clientSubscriptions.get(ctx.ws)?.delete(topic)
                },
            }
            return next(subscriptionCtx)
        }

        this.routeHandlers.push({
            handler: this.composeMiddleware([...this.globalMiddlewares, subscriptionMiddleware, ...middlewares], handler as ApiHandler),
            matchFn,
            method: 'POST', // Subscriptions use POST by convention
            middlewares: [subscriptionMiddleware, ...middlewares],
            route,
        })
    }
    ...
    emitEvent(topic: string, data: unknown): void {
        const message = constructMessage(`/events/${topic}`, data)
        const payload = JSON.stringify(message)

        if (this.subscriptions[topic]) {
            for (const ws of this.subscriptions[topic]) {
                if (ws.readyState === WebSocket.OPEN) {
                    ws.send(payload)
                }
            }
...
export type SubscriptionHandler = (ctx: SubscriptionContext, request: ApiRequest) => Promise<unknown>
...
export function createWebSocketServer(options: {
    ...
    const connectionHandler = (ws: WebSocket, req: http.IncomingMessage) => {
        ...
        if (options.authOptions && process.env[options.authOptions.noSecurityEnv || 'GARAGE44_NO_SECURITY']) {
            ...
            logger.debug('[websocket] connection established for user', {user: req.session.userid})
            manager.connections.add(ws)
        } else {
